...
$closingsigns = ')]}>'
$openingsigns = '({<['
$totals = New-Object -TypeName "System.Collections.ArrayList"
...
for($x = 0; $x -lt $data.Count-1; $x++)
{
    ...
    {
        $s=0
        for($y=$open.length-1;$y -ge 0 ;$y--)
        {
            $currentint = [int32]([char]$open[$y])
            $closechar = [char]($currentint+1)
     
            if(-not $closingsigns.Contains($closechar))
            {
                $closechar = [char]($currentint+2)
            }

            
            $m = (5*$s)
            $cs =  ($closingsigns.IndexOf($closechar)+1)

            $s= $m +  $cs 

        }

        [void]$totals.Add([int64]$s)
    }

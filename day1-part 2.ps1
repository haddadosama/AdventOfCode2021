...
$content = Invoke-WebRequest -UseBasicParsing -Uri "https://adventofcode.com/2021/day/1/input" `
  -WebSession $session `
  -Headers @{
  "method"                    = "GET"
...
$array = [System.Int32[]]$content.Content.Split("`n")
...
For ($i = 1; $i -lt $array.Length; $i++) {
  ...
  $sum2 = $array[$i - 1] + $array[$i] + $array[$i + 1]
  if ($sum1 -gt $sum2) {
    $coutlarge++
